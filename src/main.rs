...
impl Command {
    fn new(name: &str, handler: fn()) -> Command {
        Command {
...
fn main() {
    ...
    handle_command("category", categories);
}

fn input_no(prompt: &str) -> usize {
    println!("{}", prompt);
    ...
    input_string.trim().parse::<usize>().unwrap()
}

fn handle_command<const N: usize>(title: &str, commands: [Command; N]) {
    println!("{title} list");

    for i in 0..commands.len() {
        let no = i+1;
        
        println!("{}) {}", no, commands[i].name);
    }

    let command_no = input_no(&format!("select {title}: "));

    if commands.len() < command_no {
        println!("invalid command no");
        return;
    }

    commands[command_no - 1].handle();
}
...
fn week1_category() {
    let commands = [
        ...
        Command::new("sfunction", week1_function),
